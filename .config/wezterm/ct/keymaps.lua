...
local function findDuplicates(list)
	...
	for identifier, count in pairs(occurrences) do
		if count > 1 then
			table.insert(duplicates, identifier)
		end
	end
...
M.runWorkProject = wezterm.action_callback(function(w, p)
	...
	local running_session = sessions:find(function(s)
		return Str(s):starts_with(TARGET)
	end)
	...
	local choices = projects:map(function(label)
		return { label = label }
	end)
	...
	w:perform_action(
		act.InputSelector({
			...
			choices = choices,
			action = wezterm.action_callback(function(w1, p1, _, line)
				w1:perform_action(
					act.SwitchToWorkspace({
						name = TARGET .. "-" .. line,
						spawn = {
