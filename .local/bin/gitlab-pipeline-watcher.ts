...
async function sendNotification(title: string, body: string, icon: string = "info", webUrl?: string, expireAfter?: string) {
  try {
    let notifyCmd = ["kitten", "notify", `--icon=${icon}`];
    ...
    if (webUrl && result.stdout.trim() === "0") {
      try {
        await $`open ${webUrl}`;
...
function formatPipelineInfo(pipeline: any): string {
...
async function getRunningPipelines(): Promise<any[]> {
  try {
    const result = await $`glab pipeline list --username=${USERNAME} --status=running --output=json`.quiet();
...
async function getPipelineById(pipelineId: string): Promise<any | null> {
  try {
    const result = await $`glab pipeline list --username=${USERNAME} --output=json`.quiet();
    ...
    for (const pipelineId of trackedIds) {
      if (!currentRunningIds.includes(pipelineId)) {
        ...
        if (completedPipeline) {
          ...
          switch (status) {
            case "success":
              log("SUCCESS", `${getStatusEmoji(status)} Pipeline ${pipelineId} (MR ${ref}) completed successfully`);
              break;
            case "failed":
